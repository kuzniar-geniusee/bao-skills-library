---
name: jira-split
description: "Take one epic from WBS split to Jira ticket structure. Two phases. Phase A (mapping): when the WBS Jira column is empty, read the epic's MVP rows (Future excluded) and the decomposed stories from the Notion MD, group children under each WBS parent (1→N, dash format), and hand the BA a paste-ready mapping plus gap/orphan flags, then stop for the BA to paste it in. Phase B (execution): when the Jira column is filled, read it via Google Drive MCP, pull the epic's existing tickets from Jira (Epic Link), and go parent-by-parent — 1:1 renames the existing ticket, an N-split keeps the first child and creates the rest with inherited fields, joins the siblings with an `Issue split` link, and every ticket gets the fixed 4-subtask set with role-based assignees (BE/FE/QA). Returns the main ticket link per parent, waits for 'ok, next'. Does NOT write story/AC into descriptions (that is jira-fill) or write back to the WBS. Trigger: 'split [epic] to jira', 'засплітай епік в джиру', 'map and split [epic]'."
---
...
1. Read the epic's WBS section. Keep **MVP rows only** — any row flagged Future in the Phase column is never mapped.
2. Read the decomposed stories from the Notion MD.
3. For each MVP WBS parent row, group the child stories that belong to it (1→N). Children are titles only, dash-prefixed, one per line — the format the BA uses in column D.
4. Surface for BA review, do not silently resolve:
   - **gap** — an MVP WBS row with no child story.
   - **orphan** — a child story in the MD with no WBS parent.
5. Output the mapping as paste-ready text for column D, plus the flag list. **Stop.** The BA reviews, pastes into the WBS, then re-invokes (or says "go") to run Phase B.

Phase A reads the WBS and the MD only. It touches neither Jira nor the WBS file.

## Phase B — Execution (D filled)

1. Read column D for the epic via MCP → for each WBS parent row, the ordered list of target ticket names (split on blank lines).
2. Pull the epic's existing tickets from Jira via MCP (read-only, no approval): `Epic Link = [epic name]`, label `WBS`, type `Story`. This gives the parent ticket per WBS row and the fields to inherit.
3. **Go parent-by-parent, iteratively.** For each WBS parent row:
   ...
   - Once this parent's tickets exist, link the split siblings with `Issue split` (see "Split links" below). A 1:1 rename produces no link — nothing was split.
...
4. When all parents are done, give the full list of main tickets as `Story name → Jira link`.
...
The parent **Story stays Unassigned** — only the four subtasks get assignees, per the Assignee mapping below. **No `Cloners` links between split siblings** — siblings are joined with `Issue split` instead (see "Split links" below). (Existing tickets in Jira may show short BE/FE subtask names — that is a manual inconsistency, not the rule; the skill always uses the full title for all four.)
...
## Split links (`Issue split`)

When a WBS parent row splits into N ≥ 2 tickets, the siblings are linked so the split stays traceable on the board. **Ticket 1 — the ticket carrying the WBS parent row — is the source**, and every other ticket of that split is linked from it.

- **Link type:** `Issue split` (`split to` / `split from`). Never `Cloners` — that type appears only where a human cloned a ticket by hand in the UI.
- **Tool:** `jira_create_issue_link` with `inward_issue_key` = ticket 1, `outward_issue_key` = ticket 2…N, `link_type` = `Issue split`. One call per sibling, so a 1→4 split produces 3 links.
- **No comment on the link.** Traceability text belongs to `jira-fill`.
- **Star, not mesh.** Only ticket 1 → each sibling; siblings are never cross-linked. N tickets give N−1 links.
- **1:1 rename → no link.** Nothing was split, so there is nothing to trace.
- **Idempotent.** Before creating a link, read the pair's existing links (`jira_get_issue`, field `issuelinks`); if an `Issue split` link already joins them, skip it. Re-running the skill on an epic never duplicates links.
- **Verify direction once per run.** The direction of an asymmetric link type is checked, not assumed: after the first `Issue split` link of a run, read ticket 1 back and confirm the link renders with ticket 1 on the `split to` side. If it is inverted, remove it with `jira_remove_issue_link` (link id from `issuelinks`) and re-create with the keys swapped, then use the corrected order for the rest of the run.
- **Failure is reported, never worked around.** If a link call fails, report the pair to the BA and continue with the remaining tickets — a missing link never blocks the split.

### Backfill on an already-split epic

An epic split before this step existed has siblings but no links. When Phase B runs over such an epic — the tickets already carry their final names, so every parent resolves 1:1 and nothing is created — the skill still walks column D parent by parent and creates the missing `Issue split` links under the rules above. Report them per parent as `ticket 1 → sibling`; no rename, no subtask work, nothing else touched.
...
- **Phase B:** the epic's ticket structure built in Jira (parents renamed, split siblings created and joined with `Issue split`, 4 subtasks each with role-based assignees), with main-ticket links reported per parent and a final summary list.

## MCP unavailable fallback

- **Phase A:** still works if the BA pastes the WBS section and MD as text — output the mapping the same way.
- **Phase B:** if Jira MCP is down, print the structure plan (per parent: target ticket names, action, inherited fields, subtask names + intended assignees, and the `Issue split` pairs as `ticket 1 → sibling`) for the BA to apply manually.
...
- Does not set `Cloners` links, and does not set `Relates` links — the only link type it creates is `Issue split` between a split's siblings. Deciding which stories relate to which is a separate BA judgement, not part of the split. (It does set subtask assignees — see Assignee mapping.)
