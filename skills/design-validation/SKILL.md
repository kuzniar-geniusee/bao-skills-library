...
- **PC-cited** — keep the PC reference (`§n` / `WBS Rn` / decision-owner + date) inside the comment, so the designer can trace it without leaving Figma.
...
Regenerate from the settled set only: if Stage 1 dropped an item, it must not appear as a comment.
