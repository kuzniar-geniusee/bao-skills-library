...
1. **Who is the user and what do they do here?** State the action the user takes on this screen, in this flow. That is the criterion.
2. **Is this a consequence, not an action?** If the line describes a downstream record, report, transaction, notification, or a change in another epic — it is a consequence. It belongs in Related stories, not as a criterion here.
3. **Is it one action, or several glued with "and"?** Split, or drop the part that isn't this story's.
...
5. **Role / permission.** Do **not** write a standalone criterion restating who can and cannot perform the action — the role already lives in the Story statement ("As an Admin or System Admin, I want…"). Only call out a role *inside* a criterion where the behaviour itself **branches** by role (e.g. an Admin sees all groups but a Group Owner sees only their own) — and then state only that branch, not a full permission matrix.
...
- **One shared outcome -> one criterion with listed sub-conditions.** `The system must show a verification error if: 1) the code is invalid; 2) the code is expired.`
...
2. When the user submits the form with all required fields valid and a unique email, the system must create a Pending account and start email verification.
3. When the account is created, the system must set purchaser_type = B2C, assign the Learner role by default, and add the user to the default backend group.

**Validation**
4. If a required field is empty on submit, then the system must keep the user on the form and show that field's required message.
5. If a field value is outside its accepted information, then the system must show that field's validation message.
6. When the email already belongs to an account, the system must block submission and: 1) for an active account, show "This email is already associated with an account. Please check the email address or sign in to continue."; 2) for a pending B2B invite, re-send the activation link instead. `TBD` (pending-invite message)
...
**Story:** As an Admin or System Admin, I want to affiliate compliance framework categories with a course, so that its compliance requirements are enforced.

**Design:** Figma — Course Creation & Management / Compliance Categories `TBD`

**Surface:** Admin panel (System Admin, Admin)

**Acceptance Criteria**

1. To add a compliance affiliation the user must: 1) pick a state; 2) a framework in that state; 3) categories from that framework (one or more); 4) submit — and it is added to the course.
2. The framework list depends on the chosen state; the categories list depends on the chosen framework.
3. The user can add more affiliations to the same course, from any state or framework.
4. Remove — the user can remove any category they added, one at a time.
5. If the user adds a category already affiliated with the course, the system does not add it a second time; the same framework can be reused to add different categories from it.
6. When a course has no affiliations, the section shows "No compliance affiliations".

**Related stories**

1. Creating and managing compliance frameworks and their categories, and stripping an affiliation when one is deleted -> see Compliance Frameworks.
2. How "requirement met" is calculated and shown to learners -> see Compliance Report.
3. A certificate reflecting the current requirement state, and changing when an affiliation is withdrawn -> see User Learning Experience (View certificate).

Six criteria on what the admin does — add, dependent lists, add more, remove, duplicate, empty — and three Related references for everything that happens as a result (recalculation, deletion, certificate). An earlier draft of this same story ran to twelve criteria across three groups ("Affiliation / Learner records / Framework changes"); the extra six all described consequences in other entities and belonged in Related.
